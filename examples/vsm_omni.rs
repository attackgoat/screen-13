    ...
    event_loop.run(|frame| {
        let light_data = {
            ...
            LightUniformBuffer {
                ...
                projection: Mat4::perspective_lh(fov_y, 1.0, 0.1, range),
            }
        };
        let mesh_data = {
        ...
        let shadow_cube_faces_image = frame.render_graph.bind_node(
            pool.lease(
                ImageInfo::new_2d(
        ...
        //     .clear_color(0, shadow_cube_faces_image)
    ...
    T: Default + Pod,
{
    ...
    for model in models {
        for n in 0..model.mesh.indices.len() / 3 {
            ...
            let a = Vec3::from_slice(&model.mesh.positions[a_idx..a_idx + 3]);
            let b = Vec3::from_slice(&model.mesh.positions[b_idx..b_idx + 3]);
            let c = Vec3::from_slice(&model.mesh.positions[c_idx..c_idx + 3]);
