...
Most applications will want to use the `screen_13::prelude_arc::*`-style prelude because it provides concrete type aliases which do not
...
If you plan on doing things with "driver-level" smart pointers you will be using the [Ash](https://github.com/MaikKlein/ash) function
pointers and such as you normally would. In order to create some Vulkan type you will need to use a _Screen 13_-provided `create`
function, such as this one for a compute shader pipeline:

```rust
let my_spirv_code: &[u8] = ...
let my_pipeline = ComputePipeline::create(&device, ComputePipelineInfo::new(my_spirv_code)).unwrap();
```
...
```rust
// To create
let mut my_buf = Buffer::create(&device, BufferInfo {
    size: 1024,
    usage: vk::BufferUsageFlags::TRANSFER_SRC,
    can_map: true,
}).unwrap();
assert_ne!(*my_buf, vk::Buffer::null());

// To fill with data (example)
let mapped_slice: &mut [u8] = Buffer::mapped_slice_mut(&mut my_buf);
mapped_slice[0] = 0xff;
mapped_slice[1] = 0xfe;
mapped_slice[2] = 0xff;
mapped_slice[3] = 0xfe;
...
let my_img = Image::create(&device, info).unwrap();
assert_ne!(*my_img, vk::Image::null());
```
...
is an optional map of bindings that is used in conjuction with texture arrays and dynamic specialization
constants.
...
let my_pipeline = GraphicPipeline::create(&device, info, [vertex, fragment]).unwrap();
```
...
```rust
let my_buf = BufferBinding::new(my_buf);
let my_img = ImageBinding::new(my_img);
```

_Note:_ You cannot clone a binding, and the enclosed resource cannot be taken out. You may access
a mutable borrow using `get_mut()` if no shared references are alive (more on this later).
...
should only be used with the graphs they were bound to. Nodes implement `Copy` to make using them
easier.

```rust
println!("{:?}", my_buf); // BufferBinding
println!("{:?}", my_img); // ImageBinding

// Bind our resources into opaque "usize" nodes
let my_buf = my_graph.bind_node(my_buf);
let my_img = my_graph.bind_node(my_img);

// The results have unique types!
println!("{:?}", my_buf); // BufferNode
println!("{:?}", my_img); // ImageNode

// Unbind "node" back into the "binding" so we can use it again
let my_buf = my_graph.unbind_node(my_buf);
let my_img = my_graph.unbind_node(my_img);
...
graph
    ...
    .copy_buffer_to_image(buf, image);
```
...
- `submit_pass()` - Return to the `RenderPass` borrow for additional command and passes (optional)

Example:

```rust
let mut graph = RenderGraph::new();
let my_img_node = graph.bind_node(my_img_binding);
graph
    ...
    .execute(move |device, cmd_buf, bindings| unsafe {
        ...
        let my_vk_img: vk::Image = *bindings[my_img_node];
    });
```

_Note:_ If we later record additional passes onto this graph this will happen-after these two
executions.
...
Easiest way to "run this code" against "these inputs/outputs" with "these constants" I've even seen.

```rust
let shader = ComputePipelineInfo::new(my_spirv_code);
...
graph
    .record_pass("Simple quad")
    .bind_pipeline(&pipeline)
    .clear_color(0)
    .store_color(0, output)
    .draw(move |device, cmd_buf, bindings| unsafe {
        // *Do your thing here*
        // device.cmd_draw(cmd_buf, ...)
    });
```

You can see that color attachment index `0` is cleared and stored into a new concept, a leased image.
Leases are great, provide "info" such as buffer info or whatever to the hash pool and it will lease
you a ready-to-use object that is not awaiting any GPU operations. It may not be in the "perfect" layout
or whatever but that will be fixed automatically and silently.
...
- `attach_color(idx, image)` - Synonym for `load_color` + `store_color`
- `attach_depth_stencil(idx, image)`
- `clear_color(idx)` / `clear_color_value(idx, value)`
- `clear_depth_stencil(idx)` / `clear_depth_stencil_value(idx, value)`
- `load_color(idx, image)` (and `depth_stencil`)
- `resolve_color(idx, image)` (and `depth_stencil`)
- `store_color(idx, image)` (and `depth_stencil`)

_Note:_ The attachment functions also have `_as` overloads which allow specific views to be specified

Auxiallary functions:

- `set_depth_stencil(mode)` - Sets the depth/stencil state of this pass
- `set_render_area(x, y, width, height)` - Framebuffer render area
- `set_viewport(x, y, width, height, depth_range)` / `set_scissor(x, y, width, height)`
...
Small note about executing a graph and remember to keep it around a few frames so the GPU doesn't stall.
Display does this for you if you want an event loop to handle this.
