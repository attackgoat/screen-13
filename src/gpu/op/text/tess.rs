use {super::dyn_atlas::GlyphPosition, bmfont::CharPosition};

pub fn tessellate(char: impl Tess) -> [u8; 96] {
    let x1 = char.screen_x();
    let y1 = char.screen_y();
    let x2 = char.screen_x() + char.screen_width();
    let y2 = char.screen_y() + char.screen_height();

    let u1 = char.page_x() as f32;
    let u2 = (char.page_x() + char.page_width()) as f32;
    let v1 = char.page_y() as f32;
    let v2 = (char.page_y() + char.page_height()) as f32;
...
impl Tess for GlyphPosition {
    #[inline(always)]
    fn page_height(&self) -> u32 {
        self.page_rect.dims.y
    }

    #[inline(always)]
    fn page_width(&self) -> u32 {
        self.page_rect.dims.x
    }

    #[inline(always)]
    fn page_x(&self) -> u32 {
        debug_assert!(self.page_rect.pos.x >= 0);

        self.page_rect.pos.x as _
    }

    #[inline(always)]
    fn page_y(&self) -> u32 {
        debug_assert!(self.page_rect.pos.y >= 0);

        self.page_rect.pos.y as _
    }

    #[inline(always)]
    fn screen_height(&self) -> f32 {
        self.screen_rect.dims.y
    }

    #[inline(always)]
    fn screen_width(&self) -> f32 {
        self.screen_rect.dims.x
    }

    #[inline(always)]
    fn screen_x(&self) -> f32 {
        self.screen_rect.pos.x
    }

    #[inline(always)]
    fn screen_y(&self) -> f32 {
        self.screen_rect.pos.y
    }
}
